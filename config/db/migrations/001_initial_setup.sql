...
CREATE TABLE IF NOT EXISTS forms (
    ...
    response_update_window INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS form_versions (
    ...
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (form_id, revision_id),
    FOREIGN KEY (form_id) REFERENCES forms(form_id) ON DELETE CASCADE
);

-- Unique index for form_versions table
CREATE UNIQUE INDEX IF NOT EXISTS form_versions_form_id_revision_id_idx
ON form_versions (form_id, revision_id);
...
CREATE TABLE IF NOT EXISTS images (
    ...
    width INTEGER NOT NULL CHECK (width >= 0 AND width <= 740),
...
CREATE TABLE IF NOT EXISTS sections (
    ...
    UNIQUE(form_id, seq_order, revision_id),
    FOREIGN KEY (form_id) REFERENCES forms(form_id),
    FOREIGN KEY (form_id, revision_id) REFERENCES form_versions(form_id, revision_id)
);

-- Unique index for form_id and seq_order
CREATE UNIQUE INDEX IF NOT EXISTS sections_form_id_seq_order_idx
ON sections (form_id, seq_order, revision_id);
...
CREATE TABLE IF NOT EXISTS items (
    ...
    UNIQUE(form_id, title, revision_id),
...
CREATE TABLE IF NOT EXISTS choice_questions (
    question_id SERIAL PRIMARY KEY,
    type choice_question_type_enum NOT NULL,
...
CREATE TABLE IF NOT EXISTS answers (
    ...
    score INTEGER NOT NULL DEFAULT 0, -- score assigned to each answer for quiz types
    feedback TEXT,
